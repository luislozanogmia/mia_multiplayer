...
test('desktop Google connection opens a Mia tab without an external popup', async () => {
  const source = await readFile(appUrl, 'utf8');
  const start = source.indexOf('    function startGoogleAccountConnection(){');
  const end = source.indexOf("\n    grid.addEventListener", start);
  const calls = [];
  const context = {
    window: {miaNativeBrowser: {openTab: url => calls.push(['tab', url])},
      open: () => { throw new Error('Desktop must not open an external popup'); }},
  ...
  assert.deepEqual(calls, [['browser'], ['tab', 'https://accounts.google.com/test'], ['poll']]);
});
