...
(function () {
  ...
  function render(next) {
    ...
    status.textContent = tab && tab.loading ? 'Loading\u2026' : tab && tab.error ? 'Load failed' : '';
    ...
    var signature = JSON.stringify([state.activeId, state.tabs.map(function (t) { return [t.id, t.title]; })]);
    if (signature !== lastTabs) {
      ...
      state.tabs.forEach(function (t) {
        ...
        button.type = 'button'; button.textContent = t.title || 'New tab'; button.title = button.textContent;
