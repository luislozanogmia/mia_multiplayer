...
function startHermesAuth(email, provider) {
  ...
  try {
    const child = spawn(configuredHermesBinary(), ['auth', 'add', provider, '--type', 'oauth', '--no-browser'], {
      cwd: process.cwd(),
...
function runHermesLogout(provider) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      child = spawn(configuredHermesBinary(), ['auth', 'logout', provider], {
        cwd: process.cwd(),
...
function runHermesApiKeyAdd(provider, apiKey) {
  return new Promise((resolve, reject) => {
    ...
    let child;
    try {
      // Hermes securely prompts when --api-key is omitted. Feed that prompt
      // through a private stdin pipe so the secret never appears in argv,
      // the child environment, stdout, or stderr.
      child = spawn(configuredHermesBinary(), [
        'auth', 'add', provider, '--type', 'api-key',
      ], {
        cwd: process.cwd(),
        env: hermesCredentialProcessEnv(),
        stdio: ['pipe', 'ignore', 'ignore'],
        // Break any inherited controlling terminal so Python getpass reads
        // the private stdin pipe in both systemd and developer launches.
        detached: true,
      });
...
app.post('/api/settings/harness', requireAuth, (req, res) => {
  ...
  if (provider === 'openai-api' && body.apiProvider && !requestedApiProvider) {
    return res.status(400).json({ error: 'unsupported API provider' });
  }
  if (provider !== 'openai-api' && body.model !== undefined
    && !isAllowedHermesModel(provider, body.model, body.fast === true)) {
    return res.status(400).json({ error: 'unsupported model for provider' });
  }
  if (provider !== 'openai-api' && body.fast !== undefined && typeof body.fast !== 'boolean') {
  ...
  const preference = {
    provider,
    apiProvider: provider === 'openai-api' ? (requestedApiProvider || 'openai-api') : null,
    ...(provider === 'openai-api'
      ? { model: null, fast: false }
      : normalizeHermesModelSelection(provider, body.model, body.fast === true || body.model === 'fast')),
    mode,
...
function persistNativeHermesGatewaySession(conversation, eventId, storedSessionId) {
...
async function runNativeConversationAgentReply(dispatch, signal) {
  ...
  if (dispatch.targetType === 'gateway') {
      ...
      gatewayResult = await runInferenceViaHermesGateway({
        ...
        onSession: (session) => {
          ...
          if (ownsPrivateAgentConversation) {
            persistNativeHermesGatewaySession(conversation, trigger.id, session.storedSessionId);
          }
        },
        signal,
      });
    ...
    throwIfNativeDispatchUserInactive(dispatch, trigger);
    if (ownsPrivateAgentConversation) {
      const sessionOwner = persistNativeHermesGatewaySession(conversation, trigger.id, gatewayResult.storedSessionId);
