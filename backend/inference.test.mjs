...
test('provider API-key setup uses a private stdin pipe instead of process arguments', () => {
  const source = fs.readFileSync(new URL('./server.js', import.meta.url), 'utf8');
  const start = source.indexOf('function runHermesApiKeyAdd');
  const end = source.indexOf('\nfunction runHermesAuthStatus', start);
  const implementation = source.slice(start, end);
  assert.ok(start >= 0 && end > start);
  assert.doesNotMatch(implementation, /['"]--api-key['"]/);
  assert.match(implementation, /stdio:\s*\['pipe', 'ignore', 'ignore'\]/);
  assert.match(implementation, /env:\s*hermesCredentialProcessEnv\(\)/);
  assert.match(implementation, /detached:\s*true/);
  assert.match(implementation, /child\.stdin\.end\(`\$\{apiKey\}\\n`\)/);
});
