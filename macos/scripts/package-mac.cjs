...
function assertNoPrivateContent(root) {
  walkTree(root, (entry, target) => {
    ...
    if (/(?:^|\/)(?:node_modules|venv)(?:\/|$)/.test(relative)
      || relative.startsWith(`opt${path.sep}miaos${path.sep}python${path.sep}`)
      || relative.startsWith(`Contents${path.sep}Resources${path.sep}runtime${path.sep}python${path.sep}`)
