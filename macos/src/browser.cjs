...
const { WebContentsView, session, ipcMain, Menu, nativeTheme } = require("electron");
...
function createBrowser(window, trustedOrigin, log, options = {}) {
  ...
  const state = () => ({ activeId, download, tabs: [...tabs.values()].map(tab => ({
    id: tab.id, url: tab.url, title: tab.title, error: tab.error,
  ...
  function newTab(value, options = {}) {
    ...
    const tab = {
      id, view, url: "", title: typeof options.title === "string" && options.title.trim()
        ? options.title.slice(0, 500) : "New tab", error: "", timing: null, sequence: 0, vacuumElements: [],
    ...
    wc.on("did-start-navigation", (_event, _url, inPlace, mainFrame) => {
      if (mainFrame && !inPlace) { tab.sequence++; tab.timing = null; tab.documentTimingValid = true; tab.error = ""; }
      publish();
    });
