...
const PACKAGED_HERMES_BIN = PACKAGED_RUNTIME_ROOT ? path.join(PACKAGED_RUNTIME_ROOT, "bin", "hermes") : "";
const HERMES_BIN = String(app.isPackaged
  ? PACKAGED_HERMES_BIN
  : (process.env.MIAOS_HERMES_BIN || process.env.HERMES_BIN || "")).trim();
...
function preparePackagedRuntime() {
  ...
  const pythonExecutable = path.join(PACKAGED_RUNTIME_ROOT, "python", "bin", "python3.11");
  const runtimeBin = path.join(PACKAGED_RUNTIME_ROOT, "bin");
  const hermesLauncher = path.join(runtimeBin, "hermes");
  const ghostLauncher = path.join(runtimeBin, "ghost-cli");

  for (const required of [pythonExecutable, hermesLauncher, ghostLauncher]) {
  ...
  for (const name of ["python", "python3", "python3.11"]) {
    const link = path.join(venvRoot, "bin", name);
  ...
  process.env.HERMES_PYTHON = path.join(venvRoot, "bin", "python");
...
function runHermesServiceCommand(action) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let child;
    try {
      child = spawn(configuredHermesBinary(), ["gateway", action], {
...
function createApplicationMenuTemplate() {
  ...
  const template = [
    { role: "appMenu" },
    { label: "File", submenu: [{ label: "Close Window", click: () => mainWindow?.close() }] },
    { role: "editMenu" },
